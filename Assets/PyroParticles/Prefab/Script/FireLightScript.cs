            ...
            if (seed != 0)
            {
                ...
                if (fireBaseScript != null)
                {
                    if (fireBaseScript.Stopping)
                    {
                        ...
                        firePointLight.intensity =
                            Mathf.Lerp(firePointLight.intensity, 0.0f, fireBaseScript.StopPercent);
                    }
                ...
                if (setIntensity)
                {
                    float intensity = Mathf.Clamp(
                        IntensityModifier * intensityModifier2 *
                        Mathf.PerlinNoise(seed + Time.time, seed + 1 + Time.time),
